...
export default function BookCard({ book, onClick, theme }) {
  ...
  return (
    <div onClick={onClick} className="group cursor-pointer">
      <div 
        ...
        style={{
          boxShadow: theme === 'dark' 
            ...
            : '0 10px 40px rgba(0,0,0,0.15), 0 0 0 1px rgba(0, 0, 0, 0.1)'
        }}
      >
        <img
          src={book.cover}
        ...
        <div 
          className={`absolute top-3 left-3 ${statusStyle.bg} rounded-full p-2`} 
          style={{ boxShadow: statusStyle.shadow }}
        >
          {getStatusIcon(book.status)}
        </div>
        {book.rating > 0 && (
          <div 
            className="absolute top-3 right-3 rounded-full px-3 py-1.5 flex items-center gap-1.5 backdrop-blur-lg bg-gradient-to-r from-amber-500 to-orange-500"
      ...
      <div className="mt-3 px-1">
        <div className={`${currentTheme.text} font-semibold text-sm line-clamp-1`}>
          {book.title}
        </div>
        {book.episode && (
          <div className={`${currentTheme.textSecondary} text-xs mt-0.5`}>
